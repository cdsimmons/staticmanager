...
module.exports.routes = {
  ...
  // This is basically controlling which URLs are passed to angular to handle... in our case, everything, as it's a single-page app
  '/(v[1-10])/view/:id': { //(/?^.*)
    view: 'layout'
  },
  '/(v[1-10])/cms/:id': { //(/?^.*)
  ...
  // REST stuff
  'post /(v[1-10])/api/document':  'DocumentController.create',
  'get /(v[1-10])/api/document/:id':  'DocumentController.find',
  'put /(v[1-10])/api/document/:id':  'DocumentController.update',

  'get /(v[1-10])/api/js/:id':  'JsController.find',

  // Letting blueprints handle get and put... intercepting POST though to make sure it's valid, generate IDs, and return correct response...
